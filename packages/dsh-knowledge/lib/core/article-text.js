//#region src/core/article-text.ts
/** Run on a disposable clone before text and image offsets are extracted. */
function prepareArticleMedia(root) {
	for (const element of Array.from(root.querySelectorAll("video,mp-video,mp-common-videosnap,.wx_video_context,.js_video_container,.js_video_channel_container,.js_tx_video_container,.txp_player,.video_iframe,.js_video_iframe,iframe[src*=\"videoplayer\"],iframe[data-src*=\"videoplayer\"]"))) {
...
function articleImageCandidates(root, baseUrl) {
	...
	for (const element of Array.from(root.querySelectorAll("img"))) {
		const raw = [
			...
			"data-original",
			"src"
		].map((name) => element.getAttribute(name)?.trim()).find(Boolean) ?? "";
		...
		result.push({
			src,
			alt: (element.getAttribute("alt") ?? "").replace(/\s+/gu, " ").trim().slice(0, 200),
			order: result.length,
			offset
		});
...
function boundArticleText(text, maxBytes = 1048576) {
	const encoder = new TextEncoder();
	const originalByteLength = encoder.encode(text).length;
	if (originalByteLength <= maxBytes) return {
		text,
		truncated: false,
		originalByteLength
	};
...
//#endregion
export { articleImageCandidates, articleText, boundArticleText, prepareArticleMedia };
