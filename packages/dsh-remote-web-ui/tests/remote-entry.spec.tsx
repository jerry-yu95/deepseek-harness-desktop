...
afterEach(() => {
  ...
  window.localStorage.removeItem('dsh-remote-auto-open')
  vi.unstubAllGlobals()
...
describe('RemoteEntry', () => {
  it('opens the panel on trigger click: title, subtitle, QR card, hint, actions', async () => {
    ...
    expect(screen.getByText('Personal computers can create an on-demand cross-network connection')).toBeTruthy()
  ...
  it('shows the lan-required banner instead of a QR when the bind is loopback-only', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Mobile remote control' }))
    await waitFor(() => expect(screen.getByText('Need remote access from another network?')).toBeTruthy())
    expect(screen.getByText(/Do not enable this on a managed company device/u)).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Enable personal cross-network access' })).toBeTruthy()
  ...
  it('keeps the personal-public intent across the runtime restart', async () => {
    window.localStorage.setItem('dsh-remote-auto-open', 'personal-public')
    window.dshDesktop = {
      enableRemote: vi.fn(async () => 'http://127.0.0.1:3080/'),
      getRemoteStatus: vi.fn(async () => ({
        reachable: false,
        phase: 'runtime-not-ready',
        tunnel: { state: 'starting' },
      })),
    }
    mount()
    await waitFor(() => expect(screen.getByText('Verifying the personal-device public connection…')).toBeTruthy())
    expect(window.localStorage.getItem('dsh-remote-auto-open')).toBe('personal-public')
  })
