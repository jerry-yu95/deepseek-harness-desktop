...
export type PanelState =
  | { kind: 'lan-required' }
  | { kind: 'enabling-public'; error?: string }
...
export interface RemotePanelProps {
  ...
  onRefresh(): void
  onCopy(): void
  onEnablePersonalPublic(): void
  onRetryPersonalPublic(): void
 ...
 */
export function RemotePanel({ t, state, copied, onClose, onStop, onRefresh, onCopy, onEnablePersonalPublic, onRetryPersonalPublic, onPickAddress, onPickPublic }: RemotePanelProps) {
  return (
      ...
      {state.kind === 'lan-required' ? (
        <div className={css.banner} role="alert">
          ...
          <div className={css.actions}>
            <button type="button" className={css.action} onClick={onEnablePersonalPublic}>{t('action.enablePersonalPublic')}</button>
          </div>
        </div>
      ) : state.kind === 'enabling-public' ? (
        <div className={css.banner} role="status">
          <p className={css.bannerTitle}>{state.error === undefined ? t('status.enablingPublic') : t('status.publicRetrying')}</p>
          <p className={css.bannerHint}>{state.error ?? t('status.enablingHint')}</p>
          {state.error !== undefined && (
            <div className={css.actions}>
              <button type="button" className={css.action} onClick={onRetryPersonalPublic}>{t('action.retryPublic')}</button>
            </div>
          )}
        </div>
