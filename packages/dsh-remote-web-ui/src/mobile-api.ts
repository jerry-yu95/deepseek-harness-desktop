...
export function makeMobileApiRoutes(deps: MobileApiDeps): WebRoute[] {
  ...
  const handleEvents = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const heartbeat = setInterval(() => {
      ...
      try {
        res.write(': ping\n\n')
        // Embedded mobile browsers may throttle page timers. A live SSE
        // connection is a stronger presence signal than the client interval.
        service.touchDevice(deviceId)
      } catch {
