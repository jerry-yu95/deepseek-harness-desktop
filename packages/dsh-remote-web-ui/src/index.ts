...
export interface Config {
   ...
   * This is accepted only as a reference to an externally managed,
   * organization-approved authenticated gateway. The plugin never starts a
   * tunnel process itself. Malformed values are ignored with a warning.
   */
  publicBaseUrl?: string
  /** On-demand quick tunnel. Desktop enables this only in personal-device mode. */
  autoTunnel?: boolean
...
export function apply(ctx: Context, config?: Config): void {
  const desktopPersonalPublic = process.env.DSH_DESKTOP_REMOTE_MODE === 'personal-public'
  const resolved: ResolvedConfig = {
    ...
    autoTunnel: desktopPersonalPublic || (config?.autoTunnel ?? DEFAULTS.autoTunnel),
  ...
  const resolve = (): ResolvedConfig => {
    ...
    return {
      ...
      autoTunnel: desktopPersonalPublic || (value.autoTunnel ?? DEFAULTS.autoTunnel),
  ...
  tunnel.onPhase((info: TunnelInfo) => {
    ...
    if (info.phase === 'starting') service.setTunnelStatus({ state: 'starting' })
    if (info.phase === 'failed') service.setTunnelStatus({ state: 'failed', ...(info.error === undefined ? {} : { error: info.error }) })
    if (info.phase === 'stopped') service.setTunnelStatus(undefined)
  })
  ctx.effect(() => () => { tunnel.dispose() }, 'remote-web-ui: personal public tunnel')
