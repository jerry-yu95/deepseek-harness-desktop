...
export function ChatView({ session, mux, onBack }: ChatViewProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true
      if (reconcileTimerRef.current !== undefined) window.clearTimeout(reconcileTimerRef.current)
    }
  }, [session.sessionId])
  ...
  const send = useCallback(() => {
    ...
    const watermark = messages.reduce((max, message) => Math.max(max, message.seq), -1)
    setSending(true)
    const reconcile = (attempt: number): void => {
      if (attempt > 8) return
      reconcileTimerRef.current = window.setTimeout(() => {
        void loadHistory(session.sessionId).then(page => {
          const events = page.events.map(eventOf)
          setMessages(previous => foldEvents(events, previous))
          const reachedTerminal = hasTurnEndedAfter(events, watermark)
          if (!reachedTerminal) reconcile(attempt + 1)
        }, () => {
          reconcile(attempt + 1)
        })
      }, attempt === 0 ? 350 : 700)
    ...
    void prompt(session.sessionId, text).then(
      () => {
        setSending(false)
        setInput('')
        // SSE is best-effort on embedded mobile browsers. Reconcile the
        // history tail briefly so accepted prompts and terminal errors still
        // appear when the live stream is buffered or temporarily disconnected.
        reconcile(0)
      },
      (reason: unknown) => {
        setSending(false)
  ...
  }, [input, messages, sending, session.sessionId])
...
/** A tool-using turn can publish several assistant messages; only turn/end
 * means the complete final response is durable and polling may stop. */
export function hasTurnEndedAfter(events: readonly WireEvent[], watermark: number): boolean {
  return events.some(event => event.seq > watermark && event.type === 'turn/end')
}
...
function MessageRow({ message }: { message: RenderMessage }) {
  return (
    <div className={`chat-msg chat-msg-${message.kind}${message.pending === true ? ' chat-msg-pending' : ''}${message.failed === true ? ' chat-msg-failed' : ''}`}>
      ...
      {message.failed === true && (
        <span className="chat-msg-failtag">
          本次回复失败{message.failure === undefined ? '' : `：${message.failure}`}
        </span>
      )}
