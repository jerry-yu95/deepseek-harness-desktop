...
describe('foldEvents', () => {
  ...
  it('hides model-visible context injections from the mobile conversation', () => {
    const result = foldEvents([
      ...
      makeEvent('user/message', userMessageData('u-1', '真实问题'), 1),
    ])

    expect(result).toHaveLength(1)
    expect(result[0]).toMatchObject({ id: 'u-1', text: '真实问题', kind: 'user' })
  })
  ...
  it('reconciles a missed final message after later events advanced the watermark', () => {
    const live = foldEvents([
      makeEvent('user/message', userMessageData('u-1', '介绍一下'), 0),
      textChunk(0, 0, '我先看看', 1),
      makeEvent('tool/call', { turn: 0, step: 0, callId: 'c1', name: 'bash', arguments: '{}' }, 4),
      makeEvent('turn/end', { turn: 0, reason: { kind: 'completed' } }, 6),
    ])

    const reconciled = foldEvents([
      makeEvent('assistant/message', assistantMessageData('a-1', 0, 0, '这是完整的最终回答'), 3),
      makeEvent('turn/end', { turn: 0, reason: { kind: 'completed' } }, 6),
    ], live)

    expect(reconciled.filter(message => message.kind === 'assistant')).toHaveLength(1)
    expect(reconciled.find(message => message.kind === 'assistant')).toMatchObject({
      id: 'a-1',
      text: '这是完整的最终回答',
      seq: 6,
      pending: false,
