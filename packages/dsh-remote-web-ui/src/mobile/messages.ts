 ...
 *   message seq). Most events whose seq is already at or below the watermark
 *   are skipped, which makes re-applying streamed chunks idempotent. Final
 *   `assistant/message` events are replay-safe and remain authoritative even
 *   below the watermark: later tool/turn events may otherwise advance the UI
 *   past a final message missed by a buffered mobile SSE connection.
...
export interface RenderMessage {
  ...
  /** Safe, human-readable error detail when the turn exposes one. */
...
function applyUserMessage(state: FoldState, event: WireEvent): void {
  ...
  // The protocol also uses user/message for model-visible context injected by
  // plugins, skills, goals, and workspace instructions. Desktop renders those
  // as context rows; the compact mobile chat must show only real user input.
...
function applyAssistantMessage(state: FoldState, event: WireEvent): void {
  ...
  if (target === undefined && key !== undefined) {
    target = state.pendingByTurnStep.get(key) ?? state.turnStepMessage.get(key)
  }
  ...
  if (target !== undefined) {
    const next: RenderMessage = {
      ...
      seq: Math.max(target.seq, event.seq),
      time: Math.max(target.time, event.time),
      pending: false,
    }
...
function applyTurnEnd(state: FoldState, event: WireEvent): void {
  ...
  const failure = error === undefined
    ? undefined
    : pickString(error['message']) ?? pickString(error['code'])
...
export function foldEvents(events: readonly WireEvent[], existing?: readonly RenderMessage[]): RenderMessage[] {
  ...
  for (const event of sorted) {
    // A final assistant message overwrites content instead of appending it, so
    // replaying it is safe. Keep it eligible below the global watermark: a
    // later tool/call or turn/end may already have raised the row's seq while
    // the final body was lost from the live stream.
    if (event.seq <= watermark && event.type !== 'assistant/message') continue
