...
- A compact composer control for Standard/Enhanced orchestration and model-health warnings.
- An Agent Harness settings card with health dimensions, trend, anomalies, cache hit rate, manual probes, and user feedback.
- A durable `/harness on|off|status` fallback, plus `/harness run planner|reviewer|evaluator` for explicit role execution.
