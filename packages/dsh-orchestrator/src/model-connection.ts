...
export async function testModelConnection(input: { llm: LlmRuntime; provider: string; model: string; signal: AbortSignal }): Promise<ModelConnectionResult> {
...
export function classifyModelConnectionError(error: unknown, aborted = false): Pick<ModelConnectionResult, 'category' | 'detail'> {
  const message = error instanceof Error ? error.message : String(error)
  const normalized = message.toLowerCase()
  if (aborted || /abort|timeout|timed out/u.test(normalized)) return { category: 'timeout', detail: '连接测试超时；请检查网络、Base URL 和服务可用性。' }
  if (/\b(?:401|403)\b|unauthori[sz]ed|forbidden|credential|api[ _-]?key/u.test(normalized)) return { category: 'credentials', detail: '密钥或权限问题：服务拒绝了凭证，请检查 API Key、授权范围和账号状态。' }
  ...
  if (/\b404\b|not found/u.test(normalized)) return { category: 'endpoint-not-found', detail: 'API 地址、协议或路径不匹配：服务返回 404，请检查 Base URL 是否包含正确路径（常见为 /v1）。' }
  if (/\b(?:400|405|415|422)\b|bad request|unsupported|protocol/u.test(normalized)) return { category: 'protocol', detail: '响应格式不兼容：请检查提供方类型、API 协议、Base URL 和模型 ID。' }
  if (/\b429\b|rate.?limit|too many requests/u.test(normalized)) return { category: 'rate-limit', detail: '提供方已限流或额度不足；请稍后重试并检查账户额度。' }
  if (/fetch failed|econn|enotfound|network|socket|dns/u.test(normalized)) return { category: 'network', detail: '无法连接模型服务；请检查网络、代理、DNS 和 Base URL。' }
