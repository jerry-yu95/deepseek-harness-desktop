...
const SCALE_TOKENS = { '32K': 32_768, '128K': 131_072 };
const OUTPUT_RESERVE = 1_024;
const SAMPLE_COUNT = 3;
export function contextQualityExpectations(seed) {
    return {
...
export async function runContextQualityProbe(input) {
    if (!input.confirmed)
    ...
    if (contextWindow === undefined)
        throw new Error('context-quality-capacity-unknown');
    if (contextWindow < requestedInputTokens + OUTPUT_RESERVE)
        throw new Error('context-quality-capacity-insufficient');
...
async function executeSample(input, seed, requestedInputTokens) {
    ...
    for await (const chunk of input.llm.stream({ provider: input.provider, model: input.model, messages: [message], system: 'Return exactly one JSON object. Do not use markdown fences, tools, or hidden commentary.', maxTokens: OUTPUT_RESERVE, temperature: 0, signal: input.signal }))
...
function buildProbeMessage(expected, requestedInputTokens, tokenMeter) {
    ...
    while (low < high) {
        ...
        if (tokenMeter.estimateMessage(candidate) < target)
            low = middle;
        else
            high = middle - 1;
    }
...
function parseAnswer(text) {
    ...
    if (start < 0 || end < start)
        throw new Error('context-quality-result-unreadable');
...
function recall(expected, actual) { return Math.round(expected.filter(item => actual.includes(item)).length / expected.length * 100); }
