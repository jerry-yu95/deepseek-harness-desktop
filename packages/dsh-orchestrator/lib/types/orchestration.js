...
export async function workspaceFingerprint(cwd) {
    try {
        const options = { cwd, maxBuffer: 8 * 1024 * 1024, encoding: 'utf8' };
...
export async function runOrchestrationRole(request) {
    const snapshot = await loadHarness(request.cwd);
    if (snapshot === undefined)
        throw new Error('harness-not-initialized');
    if (snapshot.run.orchestration.mode !== 'enhanced' && snapshot.run.orchestration.mode !== 'adaptive')
    ...
    try {
        ...
        if (!outcome.cached)
            await recordRoleHealthSignals(request, outcome.value);
        ...
        return { ok: false, cached: false, role: request.role, ...(request.role === 'planner' ? { fallback: 'standard' } : {}), error: message };
    }
}
...
async function executeRole(engine, parent, signal, role, input) {
    ...
    const meta = { name: `harness-${role}`, description: `Run the Harness ${role} role with structured output.`, phases: [{ title: role }] };
    ...
    try {
        ...
        if (result.stopReason !== 'completed')
            throw new Error(result.stopReason === 'error' ? (result.error ?? 'workflow-error') : `workflow-${result.stopReason}`);
...
function validateRoleResult(role, value) {
    ...
    if (typeof result.summary !== 'string')
        throw new Error(`invalid-${role}-result`);
    if (role === 'planner' && Array.isArray(result.features) && Array.isArray(result.risks))
        return value;
    if (role === 'reviewer' && ['pass', 'repair'].includes(String(result.verdict)) && Array.isArray(result.findings))
        return value;
    if (role === 'evaluator' && ['complete', 'repair', 'blocked'].includes(String(result.decision)) && Array.isArray(result.featureResults))
        return value;
    throw new Error(`invalid-${role}-result`);
}
async function applyRoleResult(cwd, role, result) {
    if (role === 'planner') {
        const planner = result;
        if (planner.features.length === 0)
            throw new Error('planner-returned-no-features');
        await replaceFeatures(cwd, planner.features);
        const snapshot = await loadHarness(cwd);
        if (snapshot?.run.phase === 'planning')
            await transitionHarness(cwd, 'executing');
        return;
    }
    if (role === 'reviewer') {
        const reviewer = result;
        const snapshot = await loadHarness(cwd);
        if (reviewer.verdict === 'repair' && snapshot?.run.phase === 'evaluating')
            await transitionHarness(cwd, 'repairing');
        return;
    }
    const evaluator = result;
    for (const item of evaluator.featureResults)
        await updateFeature(cwd, item.id, item.status, item.evidence);
    const snapshot = await loadHarness(cwd);
    if (snapshot === undefined)
        return;
    if (evaluator.decision === 'complete' && snapshot.run.phase === 'evaluating')
        await transitionHarness(cwd, 'complete');
    else if (evaluator.decision === 'repair' && snapshot.run.phase === 'evaluating')
        await transitionHarness(cwd, 'repairing');
    else if (evaluator.decision === 'blocked' && snapshot.run.phase !== 'complete' && snapshot.run.phase !== 'blocked')
        await transitionHarness(cwd, 'blocked');
}
