...
async function executeDirectProbe(llm, parent, signal) {
    ...
    if (llm === undefined || provider === undefined || model === undefined)
        throw new Error('model-health-probe-unavailable');
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        for await (const chunk of llm.stream({ provider, model, messages: [createUserMessage({ content: [{ type: 'text', text: request }], source: { kind: 'user' } })], system: 'Return only the requested JSON object. Do not use markdown fences.', maxTokens: 512, temperature: 0, signal }))
        ...
        try {
            ...
            if (attempt === 1)
                throw new Error('model-health-probe-format-unreadable', { cause: error });
        }
    }
...
function parseProbeResult(text) {
    ...
    if (recognized < 3)
        throw new Error('insufficient-probe-fields');
    return {
...
function extractJsonObject(text) {
    ...
    if (start < 0)
        throw new Error('probe-json-object-missing');
    let depth = 0;
    let quoted = false;
    let escaped = false;
...
function scalarString(value) { return typeof value === 'string' || typeof value === 'number' ? String(value) : ''; }
