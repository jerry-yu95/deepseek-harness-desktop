...
async function load(cwd) {
    try {
        ...
        if (value.version !== 1 || !Array.isArray(value.events) || value.sessions === null || typeof value.sessions !== 'object')
            throw new Error('invalid-observability-ledger');
        return value;
    }
    catch (error) {
...
export async function recordRuntimeEvent(cwd, event) {
...
export async function aggregateObservability(cwd, query) {
    const ledger = await load(cwd);
    ...
    for (const event of tokenEvents)
        grouped.set(event.modelKey, [...(grouped.get(event.modelKey) ?? []), event]);
    ...
    for (const event of tokenEvents)
        dailyMap.set(event.timestamp.slice(0, 10), (dailyMap.get(event.timestamp.slice(0, 10)) ?? 0) + total(event));
...
function subtract(next, previous) { return Object.fromEntries(Object.entries(next).map(([key, value]) => [key, Math.max(0, value - (previous?.[key] ?? 0))])); }
function sumBuckets(values) { return values.reduce((sum, value) => ({ uncachedInputTokens: sum.uncachedInputTokens + value.uncachedInputTokens, outputTokens: sum.outputTokens + value.outputTokens, cacheReadTokens: sum.cacheReadTokens + value.cacheReadTokens, cacheWriteTokens: sum.cacheWriteTokens + value.cacheWriteTokens }), zero()); }
function total(value) { return value.uncachedInputTokens + value.outputTokens + value.cacheReadTokens + value.cacheWriteTokens; }
function validTimestamp(value) { return Number.isNaN(Date.parse(value)) ? new Date().toISOString() : new Date(value).toISOString(); }
function inRange(value, range) { const time = Date.parse(value); return (range.from === undefined || time >= range.from) && (range.to === undefined || time <= range.to); }
function dateRange(query) {
    const now = Date.parse(query.now ?? new Date().toISOString());
    if (query.period === 'all')
        return {};
    if (query.period === 'custom')
        return { ...(query.from === undefined ? {} : { from: Date.parse(query.from) }), ...(query.to === undefined ? {} : { to: Date.parse(query.to) }) };
    const date = new Date(now);
    if (query.period === 'today')
        return { from: Date.parse(`${date.toISOString().slice(0, 10)}T00:00:00.000Z`), to: now };
