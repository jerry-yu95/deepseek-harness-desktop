...
const paths = (cwd) => ({ root: harnessDir(cwd), run: join(harnessDir(cwd), 'run.json'), features: join(harnessDir(cwd), 'feature-list.json'), progress: join(harnessDir(cwd), 'progress.md') });
...
function validateRun(run) {
    if (run?.version !== 1 || typeof run.objective !== 'string' || !Object.hasOwn(TRANSITIONS, run.phase))
...
export async function initHarness(cwd, objective, featureTitles = []) {
    ...
    const run = { version: 1, objective: objective.trim(), phase: 'planning', createdAt: now, updatedAt: now };
