...
async function testModelConnection(input) {
	...
	const timer = setTimeout(() => timeout.abort("model-connection-timeout"), 2e4);
...
function classifyModelConnectionError(error, aborted = false) {
	const message = error instanceof Error ? error.message : String(error);
	const normalized = message.toLowerCase();
	if (aborted || /abort|timeout|timed out/u.test(normalized)) return {
		category: "timeout",
		detail: "连接测试超时；请检查网络、Base URL 和服务可用性。"
	};
	if (/\b(?:401|403)\b|unauthori[sz]ed|forbidden|credential|api[ _-]?key/u.test(normalized)) return {
		category: "credentials",
		detail: "密钥或权限问题：服务拒绝了凭证，请检查 API Key、授权范围和账号状态。"
	};
	if (/model[_ -]?not[_ -]?found|unknown model|invalid model|no such model|model.{0,80}does not exist/u.test(normalized)) return {
		category: "model-not-found",
		detail: "模型 ID 不正确，或当前密钥无权使用该模型。"
	};
	if (/\b404\b|not found/u.test(normalized)) return {
		category: "endpoint-not-found",
		detail: "API 地址、协议或路径不匹配：服务返回 404，请检查 Base URL 是否包含正确路径（常见为 /v1）。"
	};
	if (/\b(?:400|405|415|422)\b|bad request|unsupported|protocol/u.test(normalized)) return {
		category: "protocol",
		detail: "响应格式不兼容：请检查提供方类型、API 协议、Base URL 和模型 ID。"
	};
	if (/\b429\b|rate.?limit|too many requests/u.test(normalized)) return {
		category: "rate-limit",
		detail: "提供方已限流或额度不足；请稍后重试并检查账户额度。"
	};
	if (/fetch failed|econn|enotfound|network|socket|dns/u.test(normalized)) return {
		category: "network",
		detail: "无法连接模型服务；请检查网络、代理、DNS 和 Base URL。"
	};
	return {
		category: "provider",
		detail: `模型服务返回异常：${redact(message)}`
	};
}
function redact(message) {
	return message.replace(/Bearer\s+\S+/giu, "Bearer [REDACTED]").replace(/(authorization)\s*[:=]\s*[^\s,;]+/giu, "$1=[REDACTED]").replace(/(api[_-]?key|token|secret|password)\s*[:=]\s*[^\s,;]+/giu, "$1=[REDACTED]").replace(/\bsk-[A-Za-z0-9_-]+\b/gu, "[REDACTED]").slice(0, 300);
}
//#endregion
//#region src/index.ts
	...
	ctx.effect(() => ctx.connection.rpc.handle(HARNESS_RPC_CHANNEL, async (endpoint, payload, signal) => {
		try {
			...
			if (endpoint === "connection-test") {
				const route = requireCurrentModelRoute(requireLiveAgent(ctx, parseConnectionTestRequest(payload).sessionId));
				return {
					ok: true,
					value: await testModelConnection({
						llm: ctx.llm,
...
//#endregion
export { HARNESS_RPC_CHANNEL, aggregateContextQuality, aggregateObservability, appendProgress, apply, assessModelHealth, assessTask, cacheKey, cached, classifyModelConnectionError, contextQualityExpectations, contextQualityScore, createRunRecord, executeHarnessCommand, getModelHealth, harnessContext, harnessContextSync, harnessDir, initHarness, inject, loadContextQualityHistory, loadHarness, loadHealthStore, name, readCache, recordContextQualityRun, recordHealthFeedback, recordHealthSignals, recordRuntimeEvent, recordTokenSnapshot, redactSecrets, replaceFeatures, retrieveMemory, runContextQualityProbe, runModelHealthProbe, runOrchestrationRole, sanitizeTrajectory, setOrchestrationMode, stableDigest, testModelConnection, transitionHarness, updateFeature, updateOrchestration, validateAdaptiveDag, workspaceFingerprint, writeCache, writeRunRecord };
