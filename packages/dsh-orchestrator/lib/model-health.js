...
const PROBE_TTL = 360 * 60 * 1e3;
function healthPath(cwd) {
	return join(harnessDir(cwd), "model-health.json");
}
function boundedScore(score) {
	return Math.max(0, Math.min(100, Math.round(score)));
}
async function loadHealthStore(cwd) {
	try {
...
async function saveHealthStore(cwd, store) {
	...
	await writeFile(temp, `${JSON.stringify({
		...store,
		signals: store.signals.slice(-500),
		feedback: store.feedback.slice(-100)
	}, null, 2)}\n`, "utf8");
	await rename(temp, target);
}
async function recordHealthSignals(cwd, signals) {
	if (signals.length === 0) throw new Error("health-signals-required");
	const normalized = signals.map((signal) => ({
		...signal,
		timestamp: Number.isNaN(Date.parse(signal.timestamp)) ? (/* @__PURE__ */ new Date()).toISOString() : signal.timestamp,
		score: boundedScore(signal.score),
		anomaly: signal.anomaly === void 0 ? void 0 : redactSecrets(signal.anomaly).slice(0, 500)
	}));
...
async function recordHealthFeedback(cwd, feedback) {
	const store = await loadHealthStore(cwd);
	store.feedback.push({
		...feedback,
		timestamp: new Date(feedback.timestamp).toISOString(),
		...feedback.note === void 0 ? {} : { note: redactSecrets(feedback.note).slice(0, 500) }
	});
	await saveHealthStore(cwd, store);
	return assessModelHealth(feedback.modelKey, store.signals, store.feedback);
}
async function getModelHealth(cwd, modelKey) {
	const store = await loadHealthStore(cwd);
	return assessModelHealth(modelKey, store.signals, store.feedback);
}
function assessModelHealth(modelKey, allSignals, allFeedback = []) {
	...
	const baselineScore = average((signals.length >= 8 ? signals.slice(0, -3) : []).map((signal) => signal.score));
	const delta = baselineScore === void 0 ? void 0 : Math.round(score - baselineScore);
	...
	let status = "healthy";
	if (signals.length < 8 || baselineScore === void 0) status = "insufficient-data";
	...
	return {
		...
		...baselineScore === void 0 ? {} : {
			baselineScore: Math.round(baselineScore),
			delta
		},
		sampleCount: signals.length,
		dimensions,
		anomalies: signals.filter((signal) => signal.anomaly !== void 0).slice(-20).reverse().map((signal) => ({
			timestamp: signal.timestamp,
			dimension: signal.dimension,
			summary: signal.anomaly
		})),
		trend: signals.slice(-60).map((signal) => ({
			timestamp: signal.timestamp,
			score: signal.score,
			dimension: signal.dimension,
		...
		feedback: {
			normal: feedback.filter((item) => item.verdict === "normal").length,
			degraded: feedback.filter((item) => item.verdict === "degraded").length
		}
	};
}
function dimensionSummary(signals) {
	...
	return {
		...score === void 0 ? {} : { score: Math.round(score) },
		...baselineScore === void 0 ? {} : {
			baseline: Math.round(baselineScore),
			delta: Math.round((score ?? baselineScore) - baselineScore)
		},
		samples: signals.length
	};
}
function average(values) {
	return values.length === 0 ? void 0 : values.reduce((sum, value) => sum + value, 0) / values.length;
}
function standardDeviation(values) {
	const mean = average(values);
	if (mean === void 0) return 0;
	return Math.sqrt(values.reduce((sum, value) => sum + (value - mean) ** 2, 0) / values.length);
}
async function runModelHealthProbe(input) {
	const key = cacheKey("model-health-probe", {
	...
	const result = input.bypassCache === true ? {
		...
		cached: false
	} : await cached(input.cwd, "model-health", key, PROBE_CONTRACT, producer, PROBE_TTL);
	if (!result.cached) await recordHealthSignals(input.cwd, gradeProbe(input.modelKey, result.value));
	return {
		cached: result.cached,
		summary: await getModelHealth(input.cwd, input.modelKey)
	};
}
...
async function executeProbe(engine, parent, signal) {
	const run = engine.start({
		...
		meta: {
			name: "model-health",
			description: "Isolated deterministic model-health probe.",
			phases: [{ title: "model-health" }]
		},
		args: {
...
function gradeProbe(modelKey, result) {
	...
	return [
		signal("reasoning", result.logicAnswer.trim() === "42" ? 100 : 0, result.logicAnswer.trim() === "42" ? void 0 : "Deterministic logic answer mismatch"),
		signal("context", result.contextToken === "H7-KITE-29" ? 100 : 0, result.contextToken === "H7-KITE-29" ? void 0 : "Context token was not preserved"),
		signal("structuredOutput", result.structuredMarker === "structured-ok" ? 100 : 0, result.structuredMarker === "structured-ok" ? void 0 : "Structured marker mismatch"),
		signal("toolPlanning", plan.join(",") === "inspect,implement,test" ? 100 : 50, plan.join(",") === "inspect,implement,test" ? void 0 : "Tool plan order drifted"),
		signal("completeness", exactMarkers ? 100 : 40, exactMarkers ? void 0 : "Response completeness markers were missing"),
		signal("instruction", result.logicAnswer !== "" && result.contextToken !== "" ? 100 : 40, result.logicAnswer !== "" && result.contextToken !== "" ? void 0 : "Required fields were incomplete")
	];
	function signal(dimension, score, anomaly) {
		return {
			timestamp,
			modelKey,
			dimension,
			score,
			source: "probe",
			...anomaly === void 0 ? {} : { anomaly }
		};
	}
}
//#endregion
export { assessModelHealth, getModelHealth, loadHealthStore, recordHealthFeedback, recordHealthSignals, runModelHealthProbe };
