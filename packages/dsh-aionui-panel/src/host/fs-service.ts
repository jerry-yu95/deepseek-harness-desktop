...
import { readdir, readFile, realpath, stat, writeFile, rm, mkdir, rename } from 'node:fs/promises'
...
export class FsService {
  ...
  /** Create an empty file without overwriting an existing entry. */
  async createFile(root: string, rel: string): Promise<{ ok: true } | PanelError> {
    const gated = await this.gate(root)
    if (!gated.ok) return gated.error
    if (rel === '' || isGitPath(rel)) return { code: 'path-outside-root', message: 'invalid file path' }
    const resolved = await resolveInsideRoot(gated.canonical, rel)
    if (!resolved.ok) return resolved.error
    try {
  ...
  /** Create one directory without silently changing an existing entry. */
  async createDirectory(root: string, rel: string): Promise<{ ok: true } | PanelError> {
    const gated = await this.gate(root)
    if (!gated.ok) return gated.error
    if (rel === '' || isGitPath(rel)) return { code: 'path-outside-root', message: 'invalid directory path' }
    const resolved = await resolveInsideRoot(gated.canonical, rel)
    if (!resolved.ok) return resolved.error
    try {
  ...
  /** Rename or move one entry inside the same workspace. */
  async rename(root: string, rel: string, nextRel: string): Promise<{ ok: true } | PanelError> {
    const gated = await this.gate(root)
    if (!gated.ok) return gated.error
    if (rel === '' || nextRel === '' || isGitPath(rel) || isGitPath(nextRel)) {
      return { code: 'path-outside-root', message: 'invalid rename path' }
    }
    const source = await resolveInsideRoot(gated.canonical, rel)
    const target = await resolveInsideRoot(gated.canonical, nextRel)
