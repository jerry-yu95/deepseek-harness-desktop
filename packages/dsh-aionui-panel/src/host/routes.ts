...
export function registerPanelRoutes(ctx: Context, fs: FsService, git: GitService): () => void {
  ...
  const handler = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    switch (pathname) {
      ...
      case '/aionui-panel/create-file':
      case '/aionui-panel/create-directory': {
        const path = strField(payload, 'path')
        if (path === null) {
          json(res, FAIL(BAD_REQUEST))
          return
        }
        const result = pathname.endsWith('create-file')
          ? await fs.createFile(root, path)
          : await fs.createDirectory(root, path)
        json(res, 'ok' in result ? OK(result) : FAIL(result))
        return
      }
      case '/aionui-panel/rename': {
        const path = strField(payload, 'path')
        const nextPath = strField(payload, 'nextPath')
        if (path === null || nextPath === null) {
          json(res, FAIL(BAD_REQUEST))
          return
        }
        const result = await fs.rename(root, path, nextPath)
        json(res, 'ok' in result ? OK(result) : FAIL(result))
        return
      }
