...
export class PanelApi {
  ...
  createFile(root: string, path: string): Promise<PanelEnvelope<{ ok: true }>> {
    return post('/aionui-panel/create-file', { root, path })
  }

  createDirectory(root: string, path: string): Promise<PanelEnvelope<{ ok: true }>> {
    return post('/aionui-panel/create-directory', { root, path })
  }

  rename(root: string, path: string, nextPath: string): Promise<PanelEnvelope<{ ok: true }>> {
    return post('/aionui-panel/rename', { root, path, nextPath })
  }
