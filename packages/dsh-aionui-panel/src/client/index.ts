...
import { bindWorkspaceReferenceContext, createWorkspaceReferenceSource, installWorkspaceReferencePasteBridge } from './workspaceReference.ts'
...
export const inject = ['sessions', 'locale', 'inputTriggers', 'conversation']

/** Apply the browser half. */
export function apply(ctx: ClientContext): void {
  ctx.effect(() => bindWorkspaceReferenceContext(ctx), 'dsh-aionui-panel: workspace reference runtime')
  ctx.effect(() => installWorkspaceReferencePasteBridge(), 'dsh-aionui-panel: workspace reference paste bridge')
  ctx.effect(() => ctx.locale.register(NS, dictionaries), 'dsh-aionui-panel: dictionaries')
  ctx.effect(() => ctx.inputTriggers.registerSource(createWorkspaceReferenceSource(ctx, new PanelApi())), 'dsh-aionui-panel: workspace references')
