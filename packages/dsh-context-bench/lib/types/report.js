...
export function redactReportText(value) {
    return SECRET_PATTERNS.reduce((redacted, pattern) => redacted.replace(pattern, "[REDACTED]"), value.replace(ABSOLUTE_PATH, "[LOCAL_PATH]/"));
}
...
export function assertBaselineComparable(reports, baseline) {
    ...
    for (const report of reports) {
        ...
        for (const [metric, actual] of gateEntries) {
            ...
            if (!passed)
                throw new Error(`${report.fixtureId} regressed ${metric}: ${actual} vs baseline gate ${expected}`);
        }
    }
}
