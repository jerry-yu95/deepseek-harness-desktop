...
const MATERIALIZED_MESSAGE_COUNTS: Record<ContextScale, number> = {
  "8K": 64,
  "32K": 256,
  "128K": 1_024,
  "1M-policy": 1_024,
...
function positionRatio(position: "early" | "middle" | "late"): number {
  if (position === "early") return 0.1;
  if (position === "middle") return 0.5;
  return 0.9;
}

function filler(index: number, random: () => number): CorpusMessage {
...
function nearestFreeIndex(preferred: number, occupied: Set<number>, length: number): number {
  if (!occupied.has(preferred)) return preferred;
  for (let distance = 1; distance < length; distance += 1) {
    const after = preferred + distance;
    if (after < length && !occupied.has(after)) return after;
    const before = preferred - distance;
    if (before >= 0 && !occupied.has(before)) return before;
  }
...
export function buildSyntheticCorpus(
  ...
  options: { seed: number; scale?: ContextScale },
): SyntheticCorpus {
