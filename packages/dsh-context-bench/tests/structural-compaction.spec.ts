...
describe("official structural compaction", () => {
  it("commits the official bracket and replacement through ctx.compaction", async () => {
    ...
    expect(session.snapshotEvents().slice(-4).map(({ type }) => type)).toEqual(["compaction/start", "compaction/summary", "user/message", "compaction/end"]);
    expect(session.deriveMessages().filter(message => message.role !== 'system')).toHaveLength(1);
