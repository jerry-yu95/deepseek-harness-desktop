...
2. 皮肤仓库（本任务源码的存放地）：`<repository-root>`（源代码仓库）。
...
- **皮肤集合** `<repository-root>/packages/skins/<name>/`：qq98 / ths / xp / blue-fantasy，每个是
...
- 新包：`@linxin666/dsh-client-ui-skin-center`（id：`ui-skin-center`），源码放
  `<repository-root>/packages/skins/skin-center/`（含 package.json/dsh.client 声明/tsdown.config.ts/
...
- 皮肤仓库：`<repository-root>`（README 有完整架构说明）
